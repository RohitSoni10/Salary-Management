			...
			<div class="card mt-3">
				<div class="card-body">
					<div class="container text-center"></div>
  	  					<h2 >Salary Head</h2>
							   <form:form method="POST" action="Employees/saveSalary" modelAttribute="salList" >
								   <input type="hidden" value="${list.id}">
								      <input type="hidden" value="${list.employee_id}">
								         <input type="hidden" value="${list.salaryHeadId}">
										...
										<table class="table table-striped table-dark">
											...
											<tbody>
												<c:forEach var="list" items="${salaryForm}">
													<c:url var="update" value="/updateSalary">
														<c:param name="id" value="${list.id}"/>
													</c:url>
													<c:url var="delete" value="/deleteSalary">
														<c:param name="id" value="${list.id}"/>
													</c:url>
													<tr>
														<td>${list.value}</td>
									...
									<a type="button" class="btn btn-primary btn-lg btn-block">Add Salary</a>
								</form:form>
						</div>
					</div>
				</div>
