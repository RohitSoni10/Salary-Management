...
<h2 >Salary Calculation</h2>
					  	   <form:form method="POST" action="details" modelAttribute="salaryForm" >
					  	   
							    	<div class="form-control">
							           <div class="form-group">
							       		 				  <label style=font-weight:bold>Employee:</label>
							       		 				  
							       							<form:select class="custom-select" path="id" required="required" >
							       		  						<form:option value="">--Select--</form:option>
							               					<c:forEach items="${userList}" var="user1">
							      								<form:option value="${user1.id}">${user1.firstName}</form:option>
							     							</c:forEach>
							     							</form:select> 
							     							
							     					
							     					      <label style=font-weight:bold>From:</label>
											           		<form:input type="date"  class="form-control"  path="fromDate" placeholder="Salary From" required="required"/>
											 				 <label style=font-weight:bold>To:</label>
											           		<form:input type="date"  class="form-control"  path="toDate" placeholder="Salary To" required="required"/>  
										</div>
									</div>
