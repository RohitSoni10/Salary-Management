...
<h2> Designation List</h2>
			<form:form method="POST" action="designation/saveDes" modelAttribute="desList" >
				<input type="hidden" value="${list.id}">
					<table class="table table-striped table-dark">
							...
							<tbody>
								<c:forEach var="list" items="${desList}">
									<c:url var="update" value="/updateDes">
										<c:param name="id" value="${list.id}"/>
									</c:url>
									<c:url var="delete" value="/deleteDes">
										<c:param name="id" value="${list.id}"/>
									</c:url>
									<tr>
										<td>${list.id}</td>
										<td>${list.designation}</td>
										<td><a href="updateDes/${list.id}">Update</a>  <a href="deleteDes/${list.id}" onClick="if(!(confirm('Are you sure?')))  return false">Delete</a></td>   
									<tr>
								</c:forEach>
						</tbody>			
			</table>
			<a onclick="window.location.href='addDesignation'" type="button" class="btn btn-primary btn-lg btn-block">Add Designation</a>
		</form:form>
	</body>
</html>
